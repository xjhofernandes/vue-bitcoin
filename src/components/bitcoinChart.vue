...
export default {
  ...
  setup() {
    ...
    async function updateChart(periodValue) {
      ...
      let bitcointValues = valores.map(function (valor) {
        return valor["closing"];
      });
      let bitCoinDate = valores.map(function (valor) {
        return valor["date"].$date;
      });
    ...
    async function requisicaoAPI(periodValue) {
      ...
      let data = await fetch(
        "https://vue-bitcoin.herokuapp.com/updateChart",
        requestOptions
      )
